...
export default class Console extends Actor {
    ...
    constructor (client, name) {
        ...
        this.on('evaluationResult', this._asyncEvaluationHandler.bind(this))
    }

    /**
     * Function handles the response events of evaluationResult
     * 
     * @param {Object} event 
     */
    _asyncEvaluationHandler(event){
        this.evaluationResults.set(event.resultID, event)
    ...
    async evaluateJSAsync (script, ...args) {
        ...
        return new Promise((resolve, reject) => {
            // In case the response has arrived faster than the listener to the evaluationResult-${resultID} event
            if (this.evaluationResults.has(resultID)){
                let message = this.evaluationResults.get(resultID)
